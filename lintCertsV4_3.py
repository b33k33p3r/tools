...
def process_csv_file(csv_file, results_dir):
    certs_dir = os.path.join(results_dir, "certificates")
    total_certificates = 0  # Counter for total certificates
    ...
    try:
        with open(csv_file, mode='r', newline='', encoding='utf-8') as file:
            ...
            # Identify the certificate column by checking the first row of data
            first_row = next(reader, None)
            if first_row:
                for fieldname in reader.fieldnames:
                    if first_row[fieldname].startswith(("MIIF", "MIIE", "MIIG")):
                        cert_column = fieldname
                        break

            if not cert_column:
                print("No certificate column found in the CSV file.")
                return

            print(f"Using column '{cert_column}' for certificate data.")
            ...
            for i, row in enumerate(reader, start=1):
                try:
                    cert_data_b64 = row.get(cert_column, "").strip()
                    ...
                    if is_certificate_valid(cert):
                        ...
                        pem_file_path = os.path.join(certs_dir, f"{sha256_hash}.pem")
                        ...
                        total_certificates += 1  # Increment counter for each valid certificate

                except Exception as e:
                    print(f"Error processing row {i}: {str(e)}")

        print(f"\nTotal certificates to be linted: {total_certificates}\n")  # Print total count
                    ...
                    if response.status_code == 200:
                        ...
                        for revoked_cert in crl:
                            if revoked_cert.serial_number == cert.serial_number:
                                # Attempt to get the revocation reason (may not exist)
                                reason = ""
                                try:
                                    reason = revoked_cert.extensions.get_extension_for_class(x509.CRLReason).value.reason
                                except x509.ExtensionNotFound:
                                    # No reason code present, handle this case
                                    reason = "Unspecified"
...
def process_certificate(cert_path, errors_dir, revoked_dir, crls_dir, results_csv_path):
    try:
        ...
        sha256_hash = hashlib.sha256(cert.public_bytes(serialization.Encoding.DER)).hexdigest()
